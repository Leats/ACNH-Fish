...
class MissingFish extends React.Component {
  ...
  render() {
    ...
    const [catchableFish, nonCatchableFish] = splitFish(
      this.props.fishList,
      isCatchable
    );
        ...
        return (
          <li key={fish[0]}>
            <ul className="InnerList">
              ...
              <li>
                <div className="FishWrapMonth">
                  {numberToMonth(fish[2])} - {numberToMonth(fish[3])}
                  <div style={{ fontSize: 12, color: "grey" }}>{fish[5]}</div>
                </div>
              </li>
              <li>
                <div className="FishWrap">
                  {isCatchable(fish) && (
                    <Label
                      color={rightTime(fish) ? "green" : "yellow"}
                      horizontal
                    >
                      Catchable
                    </Label>
      ...
      <div className="MissingFish">
        <ul className="MissingList">
          {catchableFish.map((fish) => {
            return listElements(fish);
          })}
          {nonCatchableFish.map((fish) => {
            return listElements(fish);
          })}
        </ul>
      </div>
